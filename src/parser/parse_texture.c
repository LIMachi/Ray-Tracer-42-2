...
/*   Updated: 2017/04/14 06:17:59 by hmartzol         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <rt.h>

inline static void	parse_texture_file(char *str, void **out_and_textures_holder)
{
	t_texture			*out;
	...
	out->info_index = -1;
	textures_holder = out_and_textures_holder[1];
	ft_realpath(str, buff);
	while (++out->info_index < (unsigned long)textures_holder->nb_info)
		if (!ft_strcmp(textures_holder->path[out->info_index], buff))
			break ;
}
